...
function CurtainHalf({ side, opening }: { side: "left" | "right"; opening: boolean }) {
  ...
  return (
    <motion.div
      ...
      transition={
        opening
          ? { duration: 3.35, ease: [0.65, 0, 0.2, 1] }
          : { duration: 7.5, ease: "easeInOut", repeat: Infinity, repeatType: "mirror" }
      }
    >
      <picture>
        ...
        <motion.img
          ...
          animate={
            opening
              ? { scale: 1.045, x: isLeft ? "-1.8%" : "1.8%" }
              : { scale: [1, 1.006, 1], x: [0, isLeft ? "0.3%" : "-0.3%", 0] }
          }
          transition={
            opening
              ? { duration: 3.35, ease: [0.65, 0, 0.2, 1] }
...
function Tieback({ side, opening }: { side: "left" | "right"; opening: boolean }) {
  ...
  return (
    <motion.div
      aria-hidden="true"
      className={`pointer-events-none absolute top-[57%] z-[108] hidden h-16 w-12 sm:block ${isLeft ? "left-[calc(50%-3rem)]" : "right-[calc(50%-3rem)]"}`}
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const revealTimer = window.setTimeout(() => setReveal(true), 2200);
  ...
  return (
    <section
      className="relative h-[100dvh] min-h-[100svh] w-full overflow-hidden bg-[#2d160b] text-white"
      onClick={open}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <motion.div
          animate={opening ? { scale: 1.06, opacity: 1 } : { scale: 1, opacity: 0.12 }}
        ...
        >
          <div className="max-w-4xl">
            <motion.p
              className="text-[10px] uppercase tracking-[0.48em] text-champagne-light sm:text-xs"
            ...
            >
              <span className="h-px w-12 bg-champagne-light/55 sm:w-20" />
              <span className="text-[9px] text-champagne-light">✦</span>
              <span className="h-px w-12 bg-champagne-light/55 sm:w-20" />
            </motion.div>
            <motion.h1
              className="mt-6 font-heading text-6xl leading-[0.9] text-white drop-shadow-[0_3px_24px_rgba(0,0,0,0.42)] sm:text-8xl lg:text-[9.5rem]"
            ...
            <motion.p
              className="mt-6 font-heading text-lg italic text-white/85 sm:text-2xl"
      ...
      {visible && (
        <div className="pointer-events-auto fixed inset-0 z-[90] h-[100dvh] w-[100vw] overflow-hidden" aria-hidden="true">
          ...
          {!opening && (
            <motion.div
              className="pointer-events-none absolute inset-x-0 bottom-[9%] z-[115] flex justify-center px-6"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: [0.65, 1, 0.65], y: [10, 0, 10] }}
              transition={{ duration: 2.4, repeat: Infinity, ease: "easeInOut" }}
            >
              <div className="rounded-full border border-[#f0d1ad]/65 bg-black/25 px-6 py-3 text-[10px] font-medium uppercase tracking-[0.3em] text-white shadow-[0_8px_30px_rgba(0,0,0,0.18)] backdrop-blur-[3px]">
                Tap to Open
              </div>
            </motion.div>
          )}
          ...
          {opening && (
            <motion.div
              className="pointer-events-none absolute inset-0 z-[112] bg-[radial-gradient(circle_at_50%_50%,rgba(255,232,195,0.28),transparent_30%)]"
              initial={{ opacity: 0 }}
