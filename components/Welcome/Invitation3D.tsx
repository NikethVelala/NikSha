...
export default function Invitation3D({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const scrollTilt = Math.max(-1.8, Math.min(1.8, -d * 3));
      card.style.transform = `perspective(1400px) rotateX(${py * -1.5 + scrollTilt}deg) rotateY(${px * 1.5}deg) translateY(${Math.max(0, 1 - Math.abs(d) * 1.5) * -3}px)`;
    };
